  ...
  const filteredSkills = useMemo(() => {
    ...
    return skills.filter(
      (s) =>
        ...
        s.tags.some((tag) => tag.name.toLowerCase().includes(q))
    );
  }, [skills, searchQuery]);
      ...
      {filteredSkills.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSkills.map((skill) => {
            const exerciseCount = skill.exercises?.length || 5; // Giả sử có 5 bài
            const completedExercises = 2; // Giả sử hoàn thành 2 bài
            const progress = (completedExercises / exerciseCount) * 100;
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between text-sm text-gray-600 mb-1">
                        <span>Tiến độ</span>
                        <span>{`${completedExercises}/${exerciseCount} bài`}</span>
                      </div>
